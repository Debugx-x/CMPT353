...
def pivot_months_pandas(data):
    ...
    #month.drop_duplicates(keep='first',inplace=True)

    sum_data = data.groupby(['name','month']).aggregate('sum').reset_index()
    
    monthly = sum_data.pivot(index='name',columns='month',values = 'precipitation')
